...
%.\\[0.2cm]
%\centering{\LARGE\textbf{UNC}} \\[0.5cm]
...
{
$C_n = \frac{2(2n-1)}{n+1} C_{n-1} = \frac{1}{n+1} \binom{2n}{n} \sim \frac{4^n}{n^{3/2}\sqrt{\pi}}$

$\sigma(n) = O(\log(\log(n)))$ (number of divisors of $n$)

$F_{2n+1} = F_{n}^2 + F_{n+1}^2; F_{2n} = F_{n+1}^2 - F_{n-1}^2$
...
%\subsection{Extended Euclid} % already in diophantine
...
\subsection{C++ stuff}
\cppfile{other/cpp_stuff_short.cpp}
...
\subsection{Other}
\cppfile{other/other.cpp}
\input{other/other.tex}
\subsection{Topic}
\cppfile{other/topic.txt}
%\subsection{Template}

...
struct Node {
	pt pp; ll x0=INF, x1=-INF, y0=INF, y1=-INF;
	Node *first=0, *second=0;
	ll distance(pt p){
		ll x=min(max(x0,p.x),x1), y=min(max(y0,p.y),y1);
	...
	Node(vector<pt>&& vp):pp(vp[0]){
		...
		if(SZ(vp)>1){
			...
			int m=SZ(vp)/2; first=new Node({vp.begin(),vp.begin()+m});
...
struct KDTree {
	...
	pair<ll,pt> search(pt p, Node *node){
		if(!node->first){
			//if(p==node->pp) {INF,pt()};		//avoid query point as answer

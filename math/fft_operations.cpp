...
poly invert(poly &b, int d){
	poly c = {inv(b[0])};
	while(SZ(c)<=d){
		int j=2*SZ(c); auto bb=b; bb.resize(j); poly cb=multiply(c,bb);
		fore(i,0,SZ(cb)) cb[i]=submod(0,cb[i]);
		cb[0]=addmod(cb[0],2); c=multiply(c,cb); c.resize(j);
	}
	c.resize(d+1); return c;
}
...
pair<poly,poly> divide(poly &a, poly &b){	//returns {quotient,remainder}
	...
	poly ap=a; reverse(ALL(ap)); poly bp=b; reverse(ALL(bp));
	bp=invert(bp,m-n); poly q=multiply(ap,bp);
